...
def load_model_epoch(self, model, epoch, d12, d3, d4, d5, r):
    assert os.path.exists(
        "{}models/{}/pysparams:d12={}_d3={}_d4={}_d5={}_r={}_epo={:d}_class.h5".format(self.path, self.model_params['model_name'], d12, d3, d4, d5, r, epoch)), "Weights at epoch {:d} not found".format(epoch)
...
def train(self, model):
    ...
    for i in range(self.train_params['reload'] + 1, nb_epoch):
        ...
        text_S1, text_S2, code, queries, labels, _ = self.get_data(
            self.data_params['train_path'])
        hist = model.fit(
            [np.array(text_S1), np.array(text_S2), np.array(
                code), np.array(queries)], np.array(labels),
            shuffle=True, epochs=1, batch_size=batch_size)

        if hist.history['loss'][0] < val_loss['loss']:
            val_loss = {'loss': hist.history['loss'][0], 'epoch': i}

        print('Best: Loss = {}, Epoch = {}'.format(
        ...
        indicator_one1 = [i, [test_acc, test_f1,
                              test_recall, test_precision, loss]]
        indicator_one2 = [
            i, [dev_acc, dev_f1, dev_recall, dev_precision, loss]]
...
params = "记录:dropout12=%.3f,dropout3=%.3f,dropout4=%.3f,dropout5=%.3f,num =%.5f" % (
    d12, d3, d4, d5, r)
...
f.writelines("相应的测试性能: precision=%.3f, recall=%.3f, f1=%.3f, accuracy=%.3f" % (
    ...
    epoch_test_accuracy[max_idx]))
f.writelines('\n')
f.writelines("最大test f1由 %d-th epoch:f1: precision=%.3f, recall=%.3f, f1=%.3f, accuracy=%.3f" % (max_idx_t,
             epoch_test_precision[max_idx_t], epoch_test_recall[max_idx_t], epoch_test_f1[max_idx_t], epoch_test_accuracy[max_idx_t]))
f.writelines('\n')
f.writelines("*" * 10)
f.writelines('\n')
f.writelines("                                             ")
...
def valid(self, model, path):
    ...
    labelpred = model.predict([np.array(text_S1), np.array(
        text_S2), np.array(code), np.array(queries)], batch_size=100)
...
##### Evaluation in the development set #####
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        StandoneCode.load_model_epoch(
            model, 83, 0.25, 0.25, 0.25, 0.25, 0.0006)
        # 测试集评估
