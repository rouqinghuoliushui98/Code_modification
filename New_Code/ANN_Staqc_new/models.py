...
class CodeMF:
    def __init__(self, config):
        ...
        if not os.path.exists(os.path.join(self.config['workdir'], 'models', self.model_params['model_name'])):
...
def build(self):
    ...
    embedding_layer = Embedding(self.text_embedding.shape[0], self.text_embedding.shape[1],
                                weights=[
                                    self.text_embedding], input_length=self.text_length,
    ...
    embedding_layer = Embedding(self.text_embedding.shape[0], self.text_embedding.shape[1],
                                weights=[
                                    self.text_embedding], input_length=self.queries_length,
                                trainable=False, mask_zero=True)
    ...
    embedding_layer = Embedding(self.code_embedding.shape[0], self.code_embedding.shape[1],
                                weights=[
                                    self.code_embedding], input_length=self.code_length,
                                trainable=False, mask_zero=True)
    ...
    4. Dropout
    '''
    dropout = Dropout(self.dropout1, name='dropout_embed',
...
t1 = attention_layer(
    [text_S1_embedding_d, text_S1_embedding_d, text_S1_embedding_d])
t2 = attention_layer(
    [text_S2_embedding_d, text_S2_embedding_d, text_S2_embedding_d])
c = attention_layer([code_embedding_d, code_embedding_d, code_embedding_d])
q = attention_layer(
    [queries_embedding_d, queries_embedding_d, queries_embedding_d])
...
gmp_1 = GlobalAveragePooling1D(name='global_max_pool_column')
...
attention_trans_layer = Lambda(lambda x: K.permute_dimensions(
    x, (0, 2, 1)), name='trans_attention')
attention_transposed = attention_trans_layer(attention_out)
gmp_2 = GlobalAveragePooling1D(name='global_max_pool_row')
