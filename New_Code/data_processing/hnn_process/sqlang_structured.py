...
class SqlangParser:
    ...
    def identifyLiterals(self, tokenList):
        ...
        for tok in tokenList.tokens:
            ...
            elif tok.ttype in [sqlparse.tokens.Keyword, sqlparse.tokens.Keyword.DML] or str(tok) == "select":
                tok.ttype = KEYWORD
    ...
    def identifyTables(self, tokenList):
        ...
        for i in range(len(tokenList.tokens)):
            ...
            elif str(tok) in ["from", "join", "inner join", "left join", "right join", "full join", "cross join", "on",
                              "where", "group by", "having", "order by", "union", "intersect", "except"] and tok.ttype == sqlparse.tokens.Keyword:
